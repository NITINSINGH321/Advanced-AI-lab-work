/*
 * CS5205: Advanced Artificial Intelligence Lab - Assignment 2
 * Corrected & Optimized Version
 * * Usage:
 * ./assg02 <input_file> <N_students> <K_or_Deadline> <Mode>
 * * Modes:
 * 1: Find Min Days (Std) [Val=K]
 * 2: Find Min K (Std) [Val=Deadline]
 * 3: Find Min Days (Delayed) [Val=K]
 * 4: Find Min K (Delayed) [Val=Deadline]
 */
...
int simulate_days(int N, int K, bool delayed_handoff)
{
    ...
    while (tasks_completed_count < total_tasks)
    {
        ...
        // Tasks unlocked during this day (for Immediate Mode)
        // We collect them separately to avoid invalidating iterators
        vector<int> next_pass_additions;

        // Tasks unlocked for TOMORROW (for Delayed Mode)
        vector<int> next_day_additions;

        bool task_done_this_cycle = true;

        // Greedy Packing Loop: Repeat as long as we fit tasks in the current day
        while (task_done_this_cycle)
        {
            ...
            // Optimization: In delayed mode, we only do one pass per day because
            // no new tasks can appear until tomorrow.
            if (delayed_handoff)
                break;
        }
